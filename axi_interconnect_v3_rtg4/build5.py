...
Description:
  ...
  the Libero SoC internal tool chain infrastructure, featuring incremental
  project reuse and clean rebuild modes.
"""
...
"""
    ...
    # Assemble Sourcing Instructions for custom components (Skipped on reuse to avoid collision faults)
    design_tcl_cmds = ""
    fccc_generation_cmd = ""
...
def main():
    ...
    # Determine if execution environment must switch to an interactive presenter layer
    ...
    if interactive_sim:
        ...
        #if args.gui:
        #    sim_cmd = [args.modelsim_path, "-gui", "-do", do_macro]
        #    print(f"[INFO] Handing off control to ModelSim Graphic Interface Layout...")
        if args.gui:
            # FIX: Use Tcl's 'after' command to pause for 500ms so GUI windows can finish rendering
