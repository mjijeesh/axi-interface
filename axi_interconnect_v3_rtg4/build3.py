...
def run_libero(libero_path, tcl_script_path, target_name, mode, tb_top):
    ...
    try:
        ...
        # SUCCESS DISPATCHER: Stream the log out to console if we finished a simulation run safely
        if mode == "sim":
...
def main():
    ...
    helper_epilog = (
        ...
        "  # 2. Run Presynth Simulation and stream Verilog $display data directly to console\n"
        "  python3 build.py --family rtg4 --sim --tb_top tb_top_axi --sim_time 1500ns"
    )
    ...
    run_libero(args.libero_path, output_tcl, target, mode, args.tb_top)
