...
//   Verifies the single physical DDR3 memory interface mapped inside the
//   upper AXI space (0xE000_0000) using 4 parallel DDR3 memory chips.
//
//   Integrated Features:
//     - Realigned ports and wires to match the updated top-level layout.
//     - Probed internal fddr_init_done status hierarchically.
...
reg NSYSRESET;    
    ...
    //--------------------------------------------------------------------------
    // DDR3 PHYSICAL SYSTEM COUPLING WIRES
    //--------------------------------------------------------------------------
    wire [15:0] fddr_addr;
    wire [2:0]  fddr_ba;
    wire        fddr_cas_n;
    ...
    // Matched delay loop signals
    wire        fddr_dqs_tmatch_0_out;
    wire        fddr_dqs_tmatch_1_out;

    // Monitor initialization status hierarchically from top-level internal nets
    wire        fddr_init_done ;
    wire        clk_80mhz;
...
begin
    ...
    top uut (
      ...
      //  .board_rst             (board_rst),
        ...
        .FDDR_ADDR             (fddr_addr),
        .FDDR_BA               (fddr_ba),
        .FDDR_CAS_N            (fddr_cas_n),
        .FDDR_CKE              (fddr_cke),
        .FDDR_CLK              (fddr_clk),
        .FDDR_CLK_N            (fddr_clk_n),
        .FDDR_CS_N             (fddr_cs_n),
        .FDDR_ODT              (fddr_odt),
        .FDDR_RAS_N            (fddr_ras_n),
        .FDDR_RESET_N          (fddr_reset_n),
        .FDDR_WE_N             (fddr_we_n),
        ...
        // Loopback Match Calibration circuit
        .FDDR_DQS_TMATCH_0_IN  (fddr_dqs_tmatch_0_out),
        .FDDR_DQS_TMATCH_1_IN  (fddr_dqs_tmatch_1_out),
        .FDDR_DQS_TMATCH_0_OUT (fddr_dqs_tmatch_0_out),
        .FDDR_DQS_TMATCH_1_OUT (fddr_dqs_tmatch_1_out)
    );

    //--------------------------------------------------------------------------
    // INSTANTIATE 4 x8 DDR3 MEMORY MODULES IN PARALLEL (32-bit Wide Bus)
    //--------------------------------------------------------------------------
    // We override timing check parameters to prevent simulation limits from
    // aborting when evaluating the slow 80 MHz (12.5 ns) system clocks.
    generate
        genvar r;
        for (r = 0; r < 4; r = r + 1) begin : ddr3_chip
            ddr3 #(
                ...
                .cas_n   (fddr_cas_n),
                .we_n    (fddr_we_n),
                .dm_tdqs (fddr_dm_rdqs[r]),
                .ba      (fddr_ba),
                .addr    (fddr_addr[14:0]),  // Default JEDEC configurations use 15 Address lines
                .dq      (fddr_dq[8*r +: 8]),
                .dqs     (fddr_dqs[r]),
                .dqs_n   (fddr_dqs_n[r]),
                .tdqs_n  (),                 // Leave unconnected (x8 mode setup)
                .odt     (fddr_odt)
            );
        end
    endgenerate
    ...
    initial begin
        ...
        // WAIT STEP: Wait until the physical FDDR interface completes calibration & training
        $display("[STATUS] Waiting for FDDR Physical Controller Initialization & Calibration to complete...");
        wait(fddr_init_done === 1'b1);
        $display("[STATUS] FDDR Controller Calibration complete! Memory is online.\n");
