...
def get_sorted_generated_components(comp_dir):
    """
    Recursively scans the Libero component directory and
    sorts generated Verilog IPs bottom-up (deepest path first) to avoid dependency 
    and out-of-order module errors inside ModelSim.
    """
    ...
    # E.g. component/work/FCCC_C0/FCCC_C0_0/FCCC_C0_FCCC_C0_0_RTG4FCCC.v compiles before FCCC_C0.v
...
# ==========================================================================
def generate_tcl(target, hdl_dir, stim_dir, constraints, output_tcl, mode, tcl_dir=None, design_tcl=None):
...
"""
    ...
    design_tcl_cmds = ""
    if tcl_dir and design_tcl:
        tcl_dir_clean = tcl_dir.replace(os.sep, "/")
        design_tcl_cmds = f"""
# Sourcing pre-created IP components (such as DDR cores and FCCC configuration) in bottom-up fashion
...
"""
    ...
    # Basic FCCC generation falls back here only if a custom design script was NOT supplied
    fccc_generation_cmd = ""
    if not design_tcl:
...
"""
    ...
    # Dynamically scan only the component/work folder (reverted from targeting the entire component/ root)
    # This prevents vendor soft-IP primitives (like AutoReset_PLL.v) from being double-compiled,
    # resolving the duplicate definition load-errors, while still processing your custom SmartDesigns bottom-up.
    comp_dir = os.path.join(build_dir, target, "top", "component", "work")
    ...
    if gen_comp_files:
        ...
        for f in gen_comp_files:
            ...
            do_content += f'vlog -sv -work presynth "${{PROJECT_DIR}}/{rel_to_proj}"\n'
    ...
    # Define standard simulation generics/parameters for Microchip IPs
    vsim_generics = "-gRAM_ES_BEHAVIOR=1" if target == "rtg4" else ""

    if console_mode:
        do_content += f"""
vsim -voptargs="+acc" -L {cfg['lib_name']} -L presynth  -t 1ps {vsim_generics} presynth.{tb_top}
{vcd_cmds}log -r /*
run {sim_time}
quit -force
"""
...
def main():
    ...
    helper_epilog = (
        ...
        "  # 2. Run simulation with standard automated recursive DDR component imports\n"
        "  python3 build.py --family smartfusion2 --sim"
    )
    ...
    parser.add_argument("--design_tcl", default="FDDRC_With_INIT_recursive.tcl", help="Target pre-created hardware component IP TCL script name residing in src/tcl/.")
    ...
    # Automatically searches the target-specific folder (src/tcl/<target_family>/) first,
    # and falls back to general root (src/tcl/) second.
    design_tcl_active = None
    tcl_dir_active = None
    default_target_script = args.design_tcl
    
    # 1. Check target-specific folder (e.g. src/tcl/rtg4/)
    if os.path.isdir(target_tcl_dir) and os.path.isfile(os.path.join(target_tcl_dir, default_target_script)):
        design_tcl_active = default_target_script
        tcl_dir_active = target_tcl_dir
        print(f"[INFO] Sourcing target-specific custom IP/Sub-system design script by default: {os.path.join(target, design_tcl_active)}")
    
    # 2. Check general root folder (src/tcl/)
    elif os.path.isdir(tcl_dir) and os.path.isfile(os.path.join(tcl_dir, default_target_script)):
        design_tcl_active = default_target_script
        tcl_dir_active = tcl_dir
        print(f"[INFO] Sourcing general custom IP/Sub-system design script by default: {design_tcl_active}")
        
    # 3. Fallback Auto-Detection: Search target-specific folder first
    elif os.path.isdir(target_tcl_dir):
        available_scripts = sorted([f for f in os.listdir(target_tcl_dir) if f.endswith('.tcl')])
        if available_scripts:
            design_tcl_active = available_scripts[0]
            tcl_dir_active = target_tcl_dir
            print(f"[INFO] Sourcing auto-detected target-specific sub-system script: {os.path.join(target, design_tcl_active)}")
            
    # 4. Fallback Auto-Detection: Search general root folder second
    elif os.path.isdir(tcl_dir):
        available_scripts = sorted([f for f in os.listdir(tcl_dir) if f.endswith('.tcl') and os.path.isfile(os.path.join(tcl_dir, f))])
        if available_scripts:
            design_tcl_active = available_scripts[0]
            tcl_dir_active = tcl_dir
            print(f"[INFO] Sourcing auto-detected general sub-system script: {design_tcl_active}")
    
    if not design_tcl_active:
    ...
    generate_tcl(target, hdl_dir, stim_dir, constraints, output_tcl, mode, tcl_dir_active, design_tcl_active)
